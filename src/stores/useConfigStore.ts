...
import { loadConfig, saveApiKey, saveModel, saveThinkingMode, saveActiveTrees, saveCountdown, saveNotificationConfig, toggleTree } from '../services/configService';
...
interface ConfigState extends AppConfig {
  ...
  setNotification: (enabled: boolean, time: string) => Promise<void>;
}
...
export const useConfigStore = create<ConfigState>((set, get) => ({
  ...
  setNotification: async (enabled: boolean, time: string) => {
    await saveNotificationConfig(enabled, time);
    set({ notify_enabled: enabled, notify_time: time });
  },
}));
